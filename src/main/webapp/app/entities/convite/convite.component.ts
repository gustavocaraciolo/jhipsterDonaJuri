...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/convite'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: Convite) {
        return item.id;
    }
    registerChangeInConvites() {
        this.eventSubscriber = this.eventManager.subscribe('conviteListModification', (response) => this.loadAll());
    }
